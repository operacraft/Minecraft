...
public abstract class Minecraft implements Runnable, IPlayerUsage
{
    ...
    public void shutdownMinecraftApplet()
    {
        try
        {
            ...
            //this.getLogAgent().logInfo(Vars.debug);
    ...
    {
    	//System.out.println("click " + Vars.isActor);
    	if (!Vars.isActor)
    		return;
        if (par1 != 0 || this.leftClickCounter <= 0)
        {
    ...
    public void runTick()
    {
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput)
        {
                ...
                if (Keyboard.getEventKeyState())
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                            	if (Vars.isActor) //only allow changing perspective if we are actors
                            	{
            ...
            while (this.gameSettings.keyBindInventory.isPressed())
            {
            	if (Vars.isActor)
            	{
            		//System.out.println("--------- a=" + Vars.isActor + " c=" + Vars.isCamera + " p=" + Vars.isProjector + " s=" + Vars.isSpectator);
        ...
        if (this.theWorld != null)
        {
            ...
            if (!this.isGamePaused)
            {
            	if (!Vars.isServer) //we disable world rendering here for Vars.isServer to optimize server data processing
    ...
    public static void main(String[] par0ArrayOfStr)
    {
        ...
        boolean var3 = true;
        boolean var4 = false;
        String var5 = "ICAT09"; // + getSystemTime() % 1000L; //"ICAT09";//
        String var6 = var5;
        ...
        //Vars.playersName = var6;
        //System.out.println("init " + Vars.isSpectator + " " + var6);
        var1.put("fullscreen", "" + var4);
        var1.put("sessionid", var7);
        Frame var17 = new Frame();
        var17.setTitle("OPERAcraft");
