...
public class OpenSocketThread implements Runnable{
	...
	private void interpretMessage(String[] messageFromPD)
	{
		...
		{			
			...
			//for arm movement
			if (messageFromPD[1].equals("@arm"))
			{				
				Minecraft.getMinecraft().thePlayer.sendChatMessage(String.format("@mouth %s %s %s", messageFromPD[0], messageFromPD[2], messageFromPD[3]));				
			}
			//for teleporting one character at a time
			else if (messageFromPD[0].equals("@tpone") && messageFromPD.length == 3)
			{
			...
			//for torso
			if (messageFromPD[1].equals("@torso"))
			{				
				Minecraft.getMinecraft().thePlayer.sendChatMessage(String.format("@torso %s %s %s", messageFromPD[0], messageFromPD[2], messageFromPD[3]));				
			}
			//for teleporting one character at a time
			else if (messageFromPD[0].equals("@tpone") && messageFromPD.length == 3)
			{
