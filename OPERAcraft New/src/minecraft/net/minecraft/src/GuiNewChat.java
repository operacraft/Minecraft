...
public class GuiNewChat extends Gui
{
	...
	private String getAndColorUsername(String[] args)
	{
		...
		//KINECTODO this is hte place where all players receieve info from the OpenSocketThread.java sendChatMessage (compare username provided by pd-l2ork with the one stored in Vars
    ...
    public void printChatMessage(String par1Str)
    {
    	...
    	else
    	{
    		...
    		if (message.length > 2)
	    	{
	    		...
	    		else if (message[1].equals("@shoulder"))
	    		{
	    			displayWords = false;
	    			if (message.length == 4)
	    			{																		//keyword is @shoulder followed by a number
		    			String username = message[0].substring(1, message[0].length() - 1);	//retrieves the user name from the first position in the message array
		    			int shoulderState = Integer.parseInt(message[2]);						//retrieves the value or position of the shoulder
		    			double time = Double.parseDouble(message[3]);
		    			if (Vars.doesOperaCraftPlayerExist(username))						//checks if the user name is in player's OperaCraftPlayer list
		    			{
		    				Vars.changeOperaCraftShoulderState(username, shoulderState, time);				//changes the player's shoulder to the specified value
		    			}
	    			}
	    			else if (message.length == 5) 	// this happens when a client connects to the server and server issues shoulder position changes
	    											// and therefore needs to send username rather than use projector's username
	    			{
	    				String username = message[2];
		    			int shoulderState = Integer.parseInt(message[3]);						//retrieves the value or position of the shoulder
		    			double time = Double.parseDouble(message[4]);
		    			if (Vars.doesOperaCraftPlayerExist(username))						//checks if the user name is in player's OperaCraftPlayer list
		    			{
		    				Vars.changeOperaCraftShoulderState(username, shoulderState, time);				//changes the player's shoulder to the specified value
	    		...
	    		else if (message[1].equals("@leg"))
	    		{
	    			displayWords = false;
	    			if (message.length == 4)
	    			{																		//keyword is @leg followed by a number
		    			String username = message[0].substring(1, message[0].length() - 1);	//retrieves the user name from the first position in the message array
		    			int legState = Integer.parseInt(message[2]);						//retrieves the value or position of the leg
		    			double time = Double.parseDouble(message[3]);
		    			if (Vars.doesOperaCraftPlayerExist(username))						//checks if the user name is in player's OperaCraftPlayer list
		    			{
		    				Vars.changeOperaCraftLegState(username, legState, time);				//changes the player's arm to the specified value
		    			}
	    			}
	    			else if (message.length == 5) 	// this happens when a client connects to the server and server issues leg position changes
	    											// and therefore needs to send username rather than use projector's username
	    			{
	    				String username = message[2];
		    			int legState = Integer.parseInt(message[3]);						//retrieves the value or position of the leg
		    			double time = Double.parseDouble(message[4]);
		    			if (Vars.doesOperaCraftPlayerExist(username))						//checks if the user name is in player's OperaCraftPlayer list
		    			{
		    				Vars.changeOperaCraftLegState(username, legState, time);				//changes the player's leg to the specified value
		    			}
	    			}
	    		}
	    		else if (message[1].equals("@fade") && Vars.isSpectator)
        		{
