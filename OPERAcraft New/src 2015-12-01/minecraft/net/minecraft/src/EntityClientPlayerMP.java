    ...
    public void sendChatMessage(String par1Str)
    {
    	...
    	{   
	    			...
	    			if (Vars.doesOperaCraftPlayerExist(username))						//checks if the user name is in player's OperaCraftPlayer list
	    			{
	    				...
	    				if (Vars.isProjector)
	    				{
	    					...
	    					if (mouth_target != null)
	    					{
	    						...
	    						//mouth_target.playerNetServerHandler.sendPacketToPlayer(new Packet5PlayerInventory(Minecraft.getMinecraft().theWorld.getPlayerEntityByName(username).entityId, 4, block));
	    					}
	    				}
    		...
    		else if (message[1].equals("@arm"))
    		{
    			if (message.length == 4)
    			{
	    			String username = message[0].substring(1, message[0].length() - 1);
	    			int armState = Integer.parseInt(message[2]);
	    			double time = Double.parseDouble(message[3]);
	    			if (Vars.doesOperaCraftPlayerExist(username))
	    			{    			
	    				Vars.changeOperaCraftArmState(username, armState, time);
	    			}
    			}
    		}
    	}
